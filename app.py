...
async def on_fetch(request, env):
    ...
    try:
        ...
        if "/dashboard" in u and m == "GET":
            ...
            u1 = await env.DB.prepare("SELECT COUNT(*) as cnt FROM branch_inventory WHERE is_synced=FALSE").first()
            l1 = await env.DB.prepare("SELECT SUM(balance) as tot FROM loans WHERE status='ACTIVE'").first()
            inv = "Balanced" if u1["cnt"] == 0 else str(u1["cnt"]) + " unsynced"
            return ok({"inventory": inv, "exposure": l1["tot"] or 0})
